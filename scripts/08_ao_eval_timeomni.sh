#!/bin/bash
# final_I (10 tasks): AO Eval — TimeOmni, EP 1-8, Test A/B/C
set -e
...
for EVAL_EP in 1 2 3 4; do
    ...
    if [ -d "${LORA_DIR}" ] && [ ! -d "${MERGED_DIR}" ]; then
        echo "=== Merging TimeOmni AO epoch ${EVAL_EP} ==="
    ...
    if [ -d "${MERGED_DIR}" ]; then
        for test_tag in "test_sft:test_a" "test_b_ood_stock:test_b" "test_c_ood_stock_period:test_c"; do
            ...
            RES="${RESULT_BASE}/ao_ep${EVAL_EP}_${TAG}"
            smart_eval "${MERGED_DIR}" "${TMPDIR}" "${RES}" "${TASKS}" "${GPU_MEM_UTIL}"
        done
    else
        echo "[WARN] TimeOmni AO epoch ${EVAL_EP} not found"
    fi
done

echo ""
echo "=== ${FINAL} TimeOmni AO Eval Done ==="
